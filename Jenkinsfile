pipeline {
    ...
    environment {
        IMAGE_NAME = "yash5090/ferb-latin-converter"
    ...
    stages {
        ...
        stage('Checkout from Git') {                        
            steps {                                       
                git branch: 'main', url: 'https://github.com/yash509/DevSecOps-FerbLatinConverter-WebApp-Deployment.git'
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop ferb-latin-converter | true"
                sh "docker rm ferb-latin-converter | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh "docker run -d --name ferb-latin-converter -p 5000:80 ${IMAGE_NAME}:${TAG}" 
                //}
            }
        }
        ...
        stage('Switch Traffic Between Blue & Green Environment') {
            ...
            steps {
                script {
                    ...
                    withKubeConfig(caCertificate: '', clusterName: '', contextName: '', credentialsId: 'k8s', namespace: '', restrictKubeConfigAccess: false, serverUrl: '') {
                        sh '''
                            kubectl patch service ferb-latin-converter-service -p "{\\"spec\\": {\\"selector\\": {\\"app\\": \\"ferb-latin-converter\\", \\"version\\": \\"''' + newEnv + '''\\"}}}"
                        '''
                    }
